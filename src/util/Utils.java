...
//		}
	...
	public static List<Parameter> getParameterList(List<String> nameList){
		List<Parameter> para_list_complete = Arrays.asList(
				new Parameter(new Dataset("abalone", 9, 4177, ",", null, DataTypeEnum.COMPLETE)),
				new Parameter(new Dataset("adult", 15, 32561, ";", null, DataTypeEnum.COMPLETE)),
//				new Parameter(new Dataset("fd-red", 30, 250000, ",", null, DataTypeEnum.COMPLETE)),
...
//				new Parameter(new Dataset("diabetic", 30, 101766, ",", "?", DataTypeEnum.NULL_UNCERTAINTY)),
				...
				new Parameter(new Dataset("uniprot", 30, 512000, ",", "\"\"", DataTypeEnum.NULL_UNCERTAINTY)),
				new Parameter(new Dataset("china_weather", 18, 262920, ",", "NA", DataTypeEnum.NULL_UNCERTAINTY))
//				new Parameter(new Dataset("claims", 13, 96388, ",", null, DataTypeEnum.NULL_UNCERTAINTY)),
//				new Parameter(new Dataset("dblp10k", 34, 4837, ",", null, DataTypeEnum.NULL_UNCERTAINTY)),
...
//				new Parameter(new Dataset("t_biocase_identification_r91800_c38", 38, 91779, ",", null, DataTypeEnum.NULL_UNCERTAINTY))
				);
...
//	}
	...
	/**
	 * from local Json file to import FDs, then split right hand of each FD to get atomic cover.
	 * @param FDPath
	 * @return atomic cover
	 */
	public static List<FD> computeAtomicCover(String FDPath) {
		List<Object> s = readFDs(FDPath);
		List<FD> FDs = (List<FD>) s.get(1);
		return splitFDs(FDs);
	}
	...
	public static List<FD> combineFDs(List<FD> input){
		...
		for(FD fd : input) {
			...
			if(fd_map.containsKey(LHS)) {
				...
				FD fd_new = new FD(new ArrayList<>(fd.getLeftHand()), new ArrayList<>(fd.getRightHand()));//create a new fd
				fd_map.put(LHS, fd_new);
			}
		}
	...
	public static boolean isBCNFv2(List<Key> keys, List<FD> fds) {
		boolean is_BCNF = true;
		for(FD fd : fds) {//fd : X -> A
			List<String> X = fd.getLeftHand();
	...
	public static boolean is2NF(List<String> R, List<Key> keys, List<FD> fds) {
		Set<String> primeAttrs = getPrimeAttrs(keys);
		Set<String> nonPrimeAttrs = new HashSet<>(R);
		nonPrimeAttrs.removeAll(primeAttrs);
		boolean satisfied = true;
		for(FD fd : fds) {
			boolean check1 = false;//if an fd's right hand side has at least one non-prime attribute
			for(String a : fd.getRightHand()) {
				if(nonPrimeAttrs.contains(a)) {
					check1 = true;
					break;
				}
			}
			...
			if(check1) {
				boolean check2 = false;//check if left hand side of fd is subset of one of keys
				for(Key k : keys) {
					if(k.getAttributes().containsAll(fd.getLeftHand()) && k.size() != fd.getLeftHand().size()) {
						check2 = true;
						break;
					}
				}
				if(check2) {//non-prime attribute partially depends on key
					satisfied = false;
					break;
				}
			}
	...
	public static boolean is3NF(List<String> R, List<Key> keys, List<FD> fds) {
		if(!Utils.is2NF(R, keys, fds))
			return false;
		else {
			Set<String> primeAttrs = Utils.getPrimeAttrs(keys);
			Set<String> nonPrimeAttrs = new HashSet<>(R);
			nonPrimeAttrs.removeAll(primeAttrs);
			for(FD fd : fds) {
				for(String a : fd.getLeftHand()) {
					if(nonPrimeAttrs.contains(a)) {
						//non-prime attribute's transitive dependency
						return false;
					}
				}
			}
...
//		GEN.forEach(System.out::println);
	 ...
	 * @return schema
	 */
	public static List<String> getR(Parameter para){
